...
import (
	...
	"github.com/spr-networks/spr-gvisor-demo/kernel/sprnet"
	gnet "github.com/usbarmory/go-net"
	"gvisor.dev/gvisor/pkg/sentry/inet"
	"gvisor.dev/gvisor/pkg/sentry/kernel/auth"
	sentrynetstack "gvisor.dev/gvisor/pkg/sentry/socket/netstack"
	"gvisor.dev/gvisor/pkg/tcpip/stack"
)

const pluginMAC = "02:53:50:52:47:01"
...
var (
	...
	networkReport = networkStatus{Phase: "waiting-for-sentry", Owner: "gVisor Sentry netstack", Device: "virtio-net"}
	sentryIPStack *gnet.GVisorStack
)
...
// newSentryNetworkNamespace gives Sentry ownership of the same tcpip.Stack
// that the go-net VirtIO adapter will configure after DHCP. TamaGo only drives
// the queues; it does not create a second IP stack.
func newSentryNetworkNamespace(userNS *auth.UserNamespace) *inet.Namespace {
	tcpStack := stack.New(gnet.DefaultStackOptions)
	sentryIPStack = gnet.NewGVisorStack(1)
	sentryIPStack.Stack = tcpStack
	return inet.NewRootNamespace(sentrynetstack.NewStack(tcpStack, 1), nil, userNS)
}

func startInternetNetworking() {
	if sentryIPStack == nil {
		setNetworkStatus(func(s *networkStatus) { s.Phase, s.Error = "failed", "Sentry network stack was not initialized" })
		return
	}
	...
	lease, err := (sprnet.Client{
		Device: dev, MAC: mac, Hostname: "spr-gvisor-demo", Timeout: 30 * time.Second,
	}).Acquire()
	if err != nil {
		setNetworkStatus(func(s *networkStatus) { s.Phase, s.Error = "failed", err.Error() })
		return
	...
	iface := &gnet.Interface{Stack: sentryIPStack, NetworkDevice: dev}
	if err := iface.Init(lease.CIDR(), lease.MAC.String(), addrString(lease.Gateway)); err != nil {
		setNetworkStatus(func(s *networkStatus) { s.Phase, s.Error = "failed", fmt.Sprintf("configure Sentry netstack: %v", err) })
		return
	}
	iface.HandleStackErr = func(err error, tx bool) {
		setNetworkStatus(func(s *networkStatus) { s.Error = fmt.Sprintf("Sentry netstack tx=%t: %v", tx, err) })
	}
	...
	// The supervisor probe intentionally uses the Sentry-owned stack. Linux
	// tasks see this exact stack through RootNetworkNamespace.
	net.SocketFunc = sentryIPStack.Socket
...
func probeInternet() {
	...
	setNetworkStatus(func(s *networkStatus) {
		s.Phase = "online"
		s.Probe = "Sentry netstack DNS + TCP example.com:80 succeeded"
		s.Error = ""
	})
}
